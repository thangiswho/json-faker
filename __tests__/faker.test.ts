...
test('fakeInteger', () => {
  ...
  expect(parseInt(faker.fake("integer(125, 500)").toString())).toBeGreaterThanOrEqual(125);
...
test("fakeFloat", () => {
  ...
  expect(faker.fakeFloat(1000.51, 1000.511)).toBeLessThan(1000.52);
});
...
test("fakeDatetime", () => {
  ...
  expect(faker.fake("datetime").toString().length).toBe("2020-01-01 00:00:00".length);
...
test("fakerjs", () => {
  ...
  expect(faker.fake("{{company.companyName}} KEY1 {{commerce.productMaterial}} KEY2 {{database.type}} KEY3 {{finance.account}}"))
